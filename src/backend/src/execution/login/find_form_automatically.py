...
class LoginFormFinder:
    ...
    def find_login_automatically(self) -> XPaths | None:
        """
    ...
    def _find_username_field(self) -> XPath | None:
        """
        ...
        property_scores = [("@name", Score(10)), ("@id", Score(10)), ("@class", Score(2)), ("@autocomplete", Score(10)),
                           ("@placeholder", Score(3))]
        return self._find_form_field(id_aliases, tag_scores, type_scores, property_scores)

    def _find_password_field(self) -> XPath | None:
        """
    ...
    def _find_submit_button(self) -> XPath | None:
        """
        ...
        property_scores = [("@name", Score(10)), ("@id", Score(10)), ("@class", Score(2)), ("@value", Score(3)),
                           ("text()", Score(10))]
        return self._find_form_field(id_aliases, tag_scores, type_scores, property_scores)

    def _find_form_field(self, id_aliases: List[str], tag_scores: List[Tuple[str, Score]],
                         type_scores: List[Tuple[str, Score]],
                         property_scores: List[Tuple[str, Score]]) -> XPath | None:
        """
        ...
        for i, (tag_type, tag_score) in enumerate(tag_scores):
            selector_scoring = self._find_by_type(selector_scoring, tag_type, type_scores)
            selector_scoring = self._find_by_property(selector_scoring, tag_type, id_aliases, property_scores)
            selector_scoring = self._find_by_dom_text(selector_scoring, tag_type, id_aliases)
        ...
        return self._find_best_element(selector_scoring)

    def _find_best_element(self, scoring: Scoring) -> XPath | None:
        if not scoring:
            return None
        ...
        if self._driver.is_element_visible(xpath):
            return XPath(xpath)

        # Best element is not visible -> remove from scoring
        del scoring[best_element]
        return self._find_best_element(scoring)

    def _find_by_tag(self, scoring: Scoring, tag_scores: List[Tuple[str, Score]]) -> Scoring:
        """
    ...
    def _find_by_type(self, scoring: Scoring, tag_type: str,
                      type_scores: List[Tuple[str, Score]]) -> Scoring:
        """
        Finds element by type and adds it to the scoring dictionary.
        """
        for input_type, score in type_scores:
            scoring = self._find_element(scoring, XPath(f"//{tag_type}[@type='{input_type}']"), score)
        return scoring

    def _find_by_property(self, scoring: Scoring, tag_type: str, id_aliases: List[str],
                          property_scores: List[Tuple[str, Score]]) -> Scoring:
        """
                ...
                for element in elements:
                    ...
                    if alias in value.lower():
                        if "'" in value or '"' in value:
                            continue
                        scoring = self._find_element(scoring, XPath(f"//{tag_type}[{prop} = '{value}']"), score)
        return scoring

    def _find_by_dom_text(self, scoring: Scoring, tag_type: str, id_aliases: List[str]) -> Scoring:
        """
        ...
        for i, input_field in enumerate(inputs):
            def _element_contains_text(element: Element, element_scoring: Scoring):
                ...
                if any(substring in element.text.lower() for substring in id_aliases):
                    scoring = self._find_element(element_scoring, XPath(f"(//{tag_type})[{i + 1}]"), Score(2))
    ...
    def _find_by_form_child(self, scoring: Scoring):
        ...
        for element in scoring:
            ...
            if "form" in xpath:
                scoring = self._add_score_to_element(element, scoring, xpath, Score(100))
        return scoring
    ...
    def _add_score_to_element(self, element: Element, scoring: Scoring, xpath: str, score: int):
        """
